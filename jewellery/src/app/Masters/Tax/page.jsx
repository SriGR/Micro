...
const TaxMaster = () => {
    ...
    const ValidateFunction = () => {
        if (!state.TaxCode) {
            return showToast("Kindly enter the Tax Code", "warn")
        }
        else if (!state.TaxName) {
            return showToast("Kindly enter the Tax Name", "warn")
        }
    ...
    const handleSave = () => {
        setTableData([...tableData, { code: state.TaxCode, name: state.TaxName, percentage: state.TaxPercentage }]);
        dispatch({ type: "RESET" });
    };
