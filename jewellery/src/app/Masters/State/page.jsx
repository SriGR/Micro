...
const StateMasterReducers = (state, action) => {
    switch (action.type) {
        case "RESET":
            return initialState;
        default:
...
const StateMaster = () => {
    ...
    const handleSave = () => {
        setTableData([...tableData, { code: state.StateCode, name: state.StateName }]);
        dispatch({ type: "RESET" });
    };
            ...
            <section className="flex-1 h-full">
                ...
                <div className="w-full h-[110px] p-2 pt-4">
                    <div className="w-full flex justify-start items-center flex-wrap gap-[10px]">
                        ...
                        <div className="relative w-[calc(25%-10px)] h-auto flex flex-col justify-start items-start gap-[6px]">
                            <label className="w-full text-xs">State Name:</label>
                            <div className="relative w-full">
                                <input
                                    type="text"
                                    className="EntryInputField100 pr-8"
                                ...
                                {state.StateName && (
                                    <span
                                        onClick={() => dispatch({ type: "StateName", payload: "" })}
                                        className="absolute right-2 top-1/2 transform -translate-y-1/2 text-red-500 cursor-pointer"
                                    >
                                        <MdClear />
                                    </span>
